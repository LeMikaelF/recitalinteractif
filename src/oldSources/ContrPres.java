...
public class ContrPres {
    ...
    private final ChangeListener<String> pollThreadListener = (observable, oldValue, newValue) -> {
        ...
        if (newValue.contains("!")) {
            if (tglSeuil.isSelected()) {
                criticalChangeTexton(Vote.valueOf(newValue.substring(1)));
            }
            return;
        }
        int[] votes = new int[5];
    ...
    ContrPres() {
        ...
        //OldWebsocketHandlerImpl.setPollController(pollController);
    ...
    private void changeTexton(int numTexton) {
        ...
        try {
            Texton tempTexton = textonIo.readTexton(numTexton);
            if (tempTexton == null) {
            ...
            pollController.getQueue().add(Vote.NEWQUESTION);
            WebsocketHandler.setBroadcastInfo(curTexton.getNumTexton(), curTexton.getTextonLiens().size());
        } catch (Exception e) {
            FXCustomDialogs.showException(e);
            e.printStackTrace();
        }
    ...
    private void changeTexton(Vote choice) {
        changeTexton(curTexton.getTextonLiens().get(Util.ccToInt(choice)).getTo());
    }
    ...
    private void criticalChangeTexton(Vote cc) {
    ...
    private void doTimerOverAction() {
        ...
        Vote maxVotes = pollController.getPollCounter().getMax(sbVotes);
        displayAnimAndChangeTexton(ChangeAnimFactory.createTimerOverAnim(anchorPanePres,
                maxVotes, sbVotes.toString()), maxVotes);
        timerRunning.set(false);
    }

    private void displayAnimAndChangeTexton(ChangeAnim changeAnim, Vote cc) {
    ...
    private void menuHandler(ActionEvent event) {
        ...
        if (event.getSource() == menuLienA)
            changeTexton(curTexton.getTextonLiens().get(0).getTo());
        if (event.getSource() == menuLienB)
            changeTexton(curTexton.getTextonLiens().get(1).getTo());
        if (event.getSource() == menuLienC)
            changeTexton(curTexton.getTextonLiens().get(2).getTo());
        if (event.getSource() == menuLienD)
            changeTexton(curTexton.getTextonLiens().get(3).getTo());
        ...
        if (event.getSource() == menuOuvrirRecital) {
            try {
                textonIo = new XmlFileConnector(new String[]{});
                changeTexton(1);
                startRecitalTimer();
            } catch (Exception e) {
                FXCustomDialogs.showException(e);
